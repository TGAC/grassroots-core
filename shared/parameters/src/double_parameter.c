...
static bool AddDoubleParameterDetailsToJSON (const Parameter *param_p, json_t *param_json_p);
	...
	if (param_p)
		{
			...
			if (success_flag)
				{
					if (InitParameter (& (param_p -> dp_base_param), service_data_p, pt, name_s, display_name_s, description_s, options_p, bounds_p, level, check_value_fn))
						{
							if (service_data_p)
								{
...
static bool AddDoubleParameterDetailsToJSON (const Parameter *param_p, json_t *param_json_p)
{
	...
	if (double_param_p -> dp_current_value_p)
		{
			success_flag = SetJSONInteger (param_json_p, PARAM_CURRENT_VALUE_S, * (double_param_p -> dp_current_value_p));
		}
	...
	if (success_flag)
		{
			if (double_param_p -> dp_default_value_p)
				{
					success_flag = SetJSONInteger (param_json_p, PARAM_DEFAULT_VALUE_S, * (double_param_p -> dp_current_value_p));
				}
		}
