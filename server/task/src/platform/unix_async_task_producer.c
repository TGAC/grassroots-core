...
typedef struct UnixAsyncTaskProducer
{
	AsyncTaskProducer uatp_base_resource;
	pthread_mutex_t uatp_task_status_mutex;
	pthread_cond_t uatp_task_status_cond;
} UnixAsyncTaskProducer;




AsyncTaskProducer *CreateAsyncTaskProducer (void)
{
	UnixAsyncTaskProducer *producer_p = (UnixAsyncTaskProducer *) AllocMemory (sizeof (UnixAsyncTaskProducer));

	if (producer_p)
		{
			if (InitAsyncTaskProducer (& (producer_p -> uatp_base_resource)))
				{
					int res = pthread_mutex_init (& (producer_p -> uatp_task_status_mutex), NULL);

					if (res == 0)
						{
							res = pthread_cond_init (& (producer_p -> uatp_task_status_cond), NULL);

							if (res == 0)
								{
									return (& (producer_p -> uatp_base_resource));
								}		/* if (res == 0) */
							else
								{
									switch (res)
							...
							pthread_mutex_destroy (& (producer_p -> uatp_task_status_mutex));
						}		/* if (res == 0) */
					else
						{
							switch (res)
		...
		{
			PrintErrors (STM_LEVEL_SEVERE, __FILE__, __LINE__, "Failed to allocate memory for AsyncTaskProducer");
		}
...
void FreeAsyncTaskProducer (struct AsyncTaskProducer *producer_p)
{
	...
	int res = pthread_cond_destroy (& (unix_producer_p -> uatp_task_status_cond));
	...
	res = pthread_mutex_destroy (& (unix_producer_p -> uatp_task_status_mutex));
...
bool LockAsyncTaskProducer (AsyncTaskProducer *producer_p)
{
	bool success_flag = false;
	UnixAsyncTaskProducer *unix_producer_p = (UnixAsyncTaskProducer *) producer_p;
	int res = pthread_mutex_lock (& (unix_producer_p -> uatp_task_status_mutex));
...
bool UnlockAsyncTaskProducer (AsyncTaskProducer *producer_p)
{
	bool success_flag = false;
	UnixAsyncTaskProducer *unix_producer_p = (UnixAsyncTaskProducer *) producer_p;
	int res = pthread_mutex_unlock (& (unix_producer_p -> uatp_task_status_mutex));
